...
def test_registro_email_duplicado(client, usuario):
    ...
    response = client.post(
        "/register",
        json={"nombre": "Another User", "email": usuario.email, "password": "password123"},
    )
...
def test_registro_password_corto(client):
    ...
    response = client.post(
        "/register", json={"nombre": "User", "email": "user@example.com", "password": "123"}
    )
...
def test_login_exitoso(client, usuario):
    """Test login exitoso"""
    response = client.post("/login", json={"email": usuario.email, "password": "testpass123"})
...
def test_login_password_incorrecta(client, usuario):
    ...
    response = client.post("/login", json={"email": usuario.email, "password": "wrongpassword"})
...
def test_login_email_inexistente(client):
    ...
    response = client.post(
        "/login", json={"email": "noexiste@example.com", "password": "password123"}
    )
