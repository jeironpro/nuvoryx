...
from models import db, Archivo, Carpeta, Usuario
import zipfile
import io
...
def parse_size(size_str):
    ...
    try:
        ...
        factors = {'B': 1, 'BYTES': 1, 'KB': 1024, 'MB': 1024**2, 'GB': 1024**3, 'TB': 1024**4}
...
def detectar_tipo_archivo(nombre):
    ...
    # Imágenes
    if any(n.endswith(ext) for ext in ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.bmp', '.ico']):
        return 'imagen'
    ...
    # Código
    if any(n.endswith(ext) for ext in ['.html', '.css', '.js', '.jsx', '.ts', '.tsx', '.py', '.java', '.c', '.cpp', '.php', '.rb', '.go', '.rs']):
        return 'codigo'
...
def root():
    ...
    if carpeta_id:
        ...
        while temp:
            ...
            # Assuming Carpeta model has a 'padre' relationship or similar for parent object
            # If not, this line might need adjustment to use Carpeta.query.get(temp.carpeta_padre_id)
        ...
        if current_user.is_authenticated:
            ...
            total_carpetas = Carpeta.query.filter_by(carpeta_padre_id=None, usuario_id=current_user.id).count()
        ...
        if tipos_count:
            ...
            if tipo_mas_comun == 'hoja_calculo': tipo_mas_comun = 'Hoja de cálculo'
            elif tipo_mas_comun == 'presentacion': tipo_mas_comun = 'Presentación'
            else: tipo_mas_comun = tipo_mas_comun.capitalize()
    ...
    if carpeta_id:
        ...
        if current_user.is_authenticated:
            carpetas_query = Carpeta.query.filter_by(carpeta_padre_id=None, usuario_id=current_user.id).order_by(Carpeta.nombre).all()
            archivos = Archivo.query.filter_by(carpeta_id=None, usuario_id=current_user.id).order_by(Archivo.nombre_original.asc()).all()
        else:
            carpetas_query = Carpeta.query.filter_by(carpeta_padre_id=None, usuario_id=None).order_by(Carpeta.nombre).all()
            archivos = Archivo.query.filter_by(carpeta_id=None, usuario_id=None).order_by(Archivo.nombre_original.asc()).all()
...
def eliminar_multiples():
    ...
    for carpeta_id in carpetas_ids:
        ...
        if carpeta:
            # Aquí idealmente deberíamos eliminar recursivamente los archivos físicos dentro
            # Por simplicidad delegamos en cascade delete de la BD para los registros
            # y en una mejora futura borraríamos archivos físicos recursivamente.
            # Nota: cascade delete borrará los registros de archivos, pero NO los archivos físicos del disco.
            # Para borrar físicos, iteramos:
